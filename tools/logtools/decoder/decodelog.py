...
class HDF5Writer:
    ...
    def open(self):
        ...
        # Seek index for log viewer — one entry per second of decoded data.
        # Columns: [time_ns, byte_offset, overflow_count, last_raw_ts]
        # Allows viz.py to start decoding the .um4 file from any 1-second boundary.
        ds_opts_seek = {'maxshape': (None, 4), 'chunks': (100, 4), 'compression': 'gzip', 'compression_opts': 4}
        self.datasets['seek_index'] = self.h5file.create_dataset('seek_index', (0, 4), dtype='uint64', **ds_opts_seek)
...
def _process_single_file(logfile_path, output_path, args, L):
    ...
    global cr_ts_prev, elapsed, cridPrev, crid
    global epromIdString, currentEpromId, ecuMetadataString, mapblobBytes
    global rpm_avg, secs, fi_on, ri_on, fi_dur, ri_dur
    global address, showBinData, current_record_hex, current_record_address
    global headingsPrinted, msb, msb_id
    global crank_ts_history
    global global_time_ns, global_prev_timestamp, global_in_upper_half
    ...
    # Reset all per-file decode state
    cr_ts = -1
    fc_off = 0
    rc_off = 0
    map = -1
    ...
    msb = 0
    msb_id = -1
    crank_ts_history.clear()
    global_time_ns = 0
    global_prev_timestamp = -1
    global_in_upper_half = False
    gps_last_sec_time_ns = -1
    gps_first_sec_time_ns = -1
    gps_sec_count = 0
    gps_last_sec_value = -1
    gps_last_sec_file_pos = -1
    max_data_rate_bytes_per_sec = 0
    ...
    if args.format == 'h5':
        if not output_path.endswith('.h5') and not output_path.endswith('.hdf5'):
            print(f"Warning: HDF5 output file '{output_path}' does not have .h5 or .hdf5 extension", file=sys.stderr)

    # Initialize HDF5 writer if needed
    h5_writer = None

    # Open output file or use stdout (for human-readable format)
    # --range-mode skips the output file entirely so subprocess.run can capture stdout
    if args.range_mode:
        output_file = None
    elif args.format == 'hr' and output_path:
        try:
    ...
    # Apply seek state if provided (for log viewer range decoding in viz.py)
    if args.seek_offset > 0 or args.seek_time_ns > 0:
        timekeeper.time_ns = args.seek_time_ns
        timekeeper.overflow_count = args.seek_overflow
        timekeeper.last_raw_ts = args.seek_raw_ts

    # Initialize HDF5 writer now that we have Logsyms loaded
    if args.format == 'h5':
        h5_writer = HDF5Writer(output_path, L, timekeeper)
        h5_writer.open()
        print(f"# Writing HDF5 output to {output_path}")
    ...
    try:
        ...
        with open(logfile_path, "rb") as f:
            ...
            # Seek to byte offset if requested (log viewer range mode)
            if args.seek_offset > 0:
                f.seek(args.seek_offset)
                address = args.seek_offset

            # seek_index: track last second boundary emitted; -1 = not yet emitted
            seek_index_last_second = -1
            # For --max-time-s: record the starting time so we know when to stop
            seek_start_time_ns = timekeeper.time_ns
            ...
            while (True):
                ...
                # to resume decoding with correct TimeKeeper state from this exact position.
                if h5_writer:
                    current_second = timekeeper.time_ns // 1_000_000_000
                    if current_second > seek_index_last_second:
                        seek_index_last_second = current_second
                        h5_writer.append_data('seek_index', [
                            timekeeper.time_ns, record_offset,
                            timekeeper.overflow_count, timekeeper.last_raw_ts])

                # Stop decoding when the requested time window is exhausted
                if args.max_time_s > 0 and timekeeper.time_ns > seek_start_time_ns + int(args.max_time_s * 1_000_000_000):
                    break
                ...
                elif byte == L.LOGID_WP_RESET_REASON_TYPE_U16:
                    reason = int.from_bytes(read(f, L.LOGID_WP_RESET_REASON_DLEN), byteorder='little', signed=False)
                    bits = []
                    if reason & (1 << 12): bits.append("WDT_RSM")
                    if reason & (1 << 11): bits.append("HZD_SYS_RST")
                    if reason & (1 << 10): bits.append("GLITCH")
                    if reason & (1 <<  9): bits.append("SWCORE_PD")
                    if reason & (1 <<  8): bits.append("WDT_SWCORE")
                    if reason & (1 <<  7): bits.append("WDT_POWMAN")
                    if reason & (1 <<  6): bits.append("WDT_POWMAN_ASYNC")
                    if reason & (1 <<  5): bits.append("RESCUE")
                    if reason & (1 <<  3): bits.append("DP_RST")
                    if reason & (1 <<  2): bits.append("RUN_LOW")
                    if reason & (1 <<  1): bits.append("BOR")
                    if reason & (1 <<  0): bits.append("POR")
                ...
                # EP-specific events
                    ...
                    print(f"{fmt_record(recordCnt, timekeeper)} ECLK:   {eclk} kHz")

                elif byte == L.LOGID_EP_RESET_REASON_TYPE_U8:
                    reason = read(f, L.LOGID_EP_RESET_REASON_DLEN)[0]
                    bits = []
                    if reason & L.LOGID_EP_RESET_REASON_POR_MASK:      bits.append("POR")
                    if reason & L.LOGID_EP_RESET_REASON_RUN_MASK:      bits.append("RUN")
                    if reason & L.LOGID_EP_RESET_REASON_PSM_MASK:      bits.append("PSM")
                    if reason & L.LOGID_EP_RESET_REASON_WD_TIMER_MASK: bits.append("WD_TIMER")
                    if reason & L.LOGID_EP_RESET_REASON_WD_FORCE_MASK: bits.append("WD_FORCE")
                    desc = ",".join(bits) if bits else "none"
                    print(f"{fmt_record(recordCnt, timekeeper)} EP_RST: 0x{reason:02X} ({desc})")
        ...
        if h5_writer:
            h5_writer.close()
            if args.format == 'hdf5':
                print(f"# HDF5 file written successfully: {output_path}", file=sys.stderr)
...
def main():
    ...
    parser.add_argument('logfile', nargs='+', help='Input log file(s) to decode')
    ...
    # Seek/range mode arguments — used by viz.py log viewer to decode a slice of the log
    parser.add_argument('--seek-offset', type=int, default=0,
                        help='Byte offset to seek to before decoding')
    parser.add_argument('--seek-time-ns', type=int, default=0,
                        help='TimeKeeper.time_ns state at seek offset')
    parser.add_argument('--seek-overflow', type=int, default=0,
                        help='TimeKeeper.overflow_count state at seek offset')
    parser.add_argument('--seek-raw-ts', type=int, default=0,
                        help='TimeKeeper.last_raw_ts state at seek offset')
    parser.add_argument('--max-time-s', type=float, default=0.0,
                        help='Stop decoding after this many seconds past the seek point (0 = no limit)')
    parser.add_argument('--range-mode', action='store_true',
                        help='Write decoded output directly to stdout (used by viz.py log viewer)')

    # Parse arguments
    args = parser.parse_args()

    # Multi-file constraints
    if len(args.logfile) > 1:
        if args.output:
            parser.error("--output cannot be used with multiple input files")
        if args.seek_offset or args.seek_time_ns or args.seek_overflow or args.seek_raw_ts:
            parser.error("--seek-* options cannot be used with multiple input files")
        if args.range_mode:
            parser.error("--range-mode cannot be used with multiple input files")

    # Validate HDF5 availability once up front
    if args.format == 'h5' and not HDF5_AVAILABLE:
    ...
    # Process each file
    errors = 0
    for logfile_path in args.logfile:
        # Auto-generate output filename if not specified
        if args.output:
            output_path = args.output
        else:
            base_name = logfile_path
            for ext in ('.um4', '.hr', '.h5', '.hdf5', '.bin'):
                if base_name.lower().endswith(ext):
                    base_name = base_name[:-len(ext)]
                    break
            if args.format == 'hr':
                output_path = base_name + '.hr'
            else:  # h5
                output_path = base_name + '.h5'

        if len(args.logfile) > 1:
            print(f"# {logfile_path} -> {output_path}", file=sys.stderr)

        ret = _process_single_file(logfile_path, output_path, args, L)
        if ret:
            errors += 1
