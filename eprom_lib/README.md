...
If nothing else, these JSON description files might be useful for people on the internet trying to figure out what a particular Aprila EPROM part number might be used for.
...
One thing that could help this project would be if people who really understand the differences between the factory EPROMs could error check or otherwise improve the written descriptions of the EPROMs in this library.
...
But if you really wanted to find and fill a umod4 with EPROM images, it's got a lot of space.
...
In essence, these scrambled EPROMs were freely copyable, but useless without their descrambling board.
But once a rider owned a single descrambler board, any scrambled EPROM could be duplicated in the normal fashion using an EPROM reader/writer, then installed into the daughterboard and used in an ECU.
So 'scrambling' was certainly not the ultimate in security.
...
The codebase is set up to plug one in, should you choose to write it.

Honestly, it took a winter storm, a day-long power failure resulting in no internet to give me time with no distractions to imagine what the daughterboard had to be doing.
Then, maybe a week to do some experiments and work it out precisely.
