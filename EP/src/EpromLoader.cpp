...
uint8_t EpromLoader::loadImage(const char* imageName)
{
  ...
  if (!epromDoc) {
    ...
    return LOG_EP_LOAD_ERR_NOTFOUND;
  }
  else {
...
uint8_t EpromLoader::loadRange(bsonDoc_t epromDoc, uint32_t startOffset, uint32_t length)
{
  ...
  enqueue(LOG_EP_LOAD_ADDR, startOffset & 0xFF);
  enqueue(LOG_EP_LOAD_ADDR, (startOffset >> 8) & 0xFF);
  enqueue(LOG_EP_LOAD_LEN, length & 0xFF);
  enqueue(LOG_EP_LOAD_LEN, (length >> 8) & 0xFF);
  ...
  if (!found) {
    ...
    return LOG_EP_LOAD_ERR_NODAUGHTERBOARDKEY;
  }
  ...
  if (!found || (mem_element.elementType != BSON_TYPE_EMBEDDED_DOC)) {
    ...
    return LOG_EP_LOAD_ERR_NOMEMKEY;
  }
  ...
  if (err) {
    ...
    return LOG_EP_LOAD_ERR_BADMEMINFO;
  }
...
uint8_t EpromLoader::getMemInfo(bsonDoc_t memDoc, meminfo_t& meminfo)
{
  ...
  if (!found || (e.elementType != BSON_TYPE_INT32)) {
    ...
    return LOG_EP_LOAD_ERR_MISSING_KEY_START;
  }
  ...
  if (!found || (e.elementType != BSON_TYPE_INT32)) {
    ...
    return LOG_EP_LOAD_ERR_MISSING_KEY_LENGTH;
  }
  ...
  if (!found) {
    ...
    return LOG_EP_LOAD_ERR_MISSING_KEY_M3;
  }
  ...
  if (!((e.elementType != BSON_TYPE_INT32) || (e.elementType != BSON_TYPE_INT64))) {
    ...
    return LOG_EP_LOAD_ERR_BAD_M3_BSON_TYPE;
  }
  if (e.elementType == BSON_TYPE_INT64) {
    ...
    if (ms_word != 0) {
      ...
      return LOG_EP_LOAD_ERR_BAD_M3_VALUE;
    }
  }
  ...
  if (!found || (e.elementType != BSON_TYPE_BINARY_DATA)) {
    ...
    return LOG_EP_LOAD_ERR_NOBINKEY;
  }
  ...
  if (length != 32768) {
    ...
    return LOG_EP_LOAD_ERR_BADBINLENGTH;
  }
  ...
  if (binaryDataSubtype != 0x00) {
    ...
    return LOG_EP_LOAD_ERR_BADBINSUBTYPE;
  }
