...
// -----------------------------------------------------------------------------------------------------
    ...
    // The EP.GPIO24 is the EP to WP TX line, so the WP will always treat it as an input.
    // The WP may be annoyed by seeing a clock on that line instead of a UART signal,
    // but it will not be electrically harmful to the WP!
    #if 0
        // Use GPIO24/GPOUT2 for CLKOUT:
        #define CLKOUT_GPIO             24
        #define CLKOUT_SOURCE           CLOCKS_CLK_GPOUT2_CTRL_AUXSRC_VALUE_CLK_SYS
    #else
        // This is the new transmit-only EP to WP 16-bit UART mechanism
        #define EP_TO_WP_TX_GPIO        24
