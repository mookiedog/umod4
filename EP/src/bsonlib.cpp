...
bool Bson::findElement(uint8_t* docP, const char* elementName, element_t &e)
{
    ...
    if (*docEndP != 0x00) {
        //printf("%s: Error: document does not end with a zero byte!\n", __FUNCTION__);
    }
    else {
        //printf("%s: Doc ends with zero byte!\n", __FUNCTION__);
    }
    while ((*elementListP != 0x00) && (elementListP < docEndP)) {
        ...
        //printf("%s: Skipping %d bytes to next element\n", __FUNCTION__, skipCount);
    }
