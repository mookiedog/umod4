...
{
    if (gpio == GPS_PPS_PIN) {
        // format the dataWord as length 1, with LOGID in bits 8..15
        uint32_t dataWord = (LOGID_WP_GPS_PPS_TYPE_V << 8) | 1;
        if (logger) logger->logData_fromISR(dataWord);
    }
}
...
Gps::Gps(Uart* _uart) /*: UartCallback()*/
{
    ...
    printf("%s: PPS ISR will be serviced by RP2350 core %d\n", __FUNCTION__, get_core_num());
}
    ...
    if (((fixType==2) || (fixType==3)) && (timeValid && dateValid)) {
        ...
        if (logger) {
            ...
            if (moving && (fixType>=2)) {
                ...
                logger->logData(LOGID_WP_CSECS_TYPE_U8, LOGID_WP_CSECS_DLEN, (uint8_t*)&centis);
            }
        }
    }
